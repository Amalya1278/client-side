...
export const EditUser = () => {
  ...
  const {register,handleSubmit,formState:{errors},reset}=useForm<IUser>()//vor tvyalnery arden linen nor edit anenk
  const {id}=useParams()
  const handleUpdate:SubmitHandler<IUser>=(value)=>{
    value.id=Number(id)
    updateUser(value).then(response=>{navigate("/")}) //inputneri mej tvyalnery poxelu hamar

  }
  useEffect(()=>{
     getUserById(Number(id)).then(response=>{
     reset(response)}) //inputneri mej vor lracven naxnakan tvyalnery
  },[])
        ...
        <div className="bg-white shadow-lg rounded-lg p-8 w-full max-w-md">
          ...
          onSubmit={handleSubmit(handleUpdate)}>
            <div className="mb-4">
              ...
              <input
              {...register("name",{required:"please fill input"})}
                type="text"
                id="name"
            ...
            <div className="mb-4">
              ...
              {errors.name && <p className="text-red-400">{errors.name.message}</p>}
              <input
              {...register("surname",{required:"please fill input"})}
                type="text"
                id="surname"
                className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"
            ...
            <div className="mb-4">
              ...
              {errors.name && <p className="text-red-400">{errors.name.message}</p>}
              <input
              {...register("age",{pattern:{value:/^\d+$/,message:"enter number"}})}
                type="number"
                id="age"
                className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"
            ...
            <div className="mb-6">
              ...
              {errors.name && <p className="text-red-400">{errors.name.message}</p>}
              <input
              {...register("salary",{required:"please fill input"})}
                type="number"
                id="salary"
                className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"
            ...
            <div className="flex justify-between">
              <button
                type="submit" //button type-y request chi ugharkum
